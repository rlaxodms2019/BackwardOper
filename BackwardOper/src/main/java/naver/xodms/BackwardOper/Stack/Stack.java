package naver.xodms.BackwardOper.Stack;


import naver.xodms.BackwardOper.Stack.StackLinkSlot;
...
int inSlotIndex = (this.slotSize - (floorDownCount % this.slotSize) - 1);// 찾은 슬롯에서의 해당 
//멤버 위치입니다.
for(int i = floorDownCount / this.slotSize ; i > 0; --i)
{// i는 찾아 내려갈 슬롯 깊이 입니다.
taskSlot = taskSlot.getFrontSlot();// 인덱스에 해당하는 데이터를 가지고 있는 슬롯을 만날 
//때까지 내려갑니다.
}
